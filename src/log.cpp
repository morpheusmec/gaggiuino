...
void simpleLog(const char* msg, ...) //NOLINT(cert-dcl50-cpp)
{
  char msgBuf[LOG_MAX_PREFIX_LEN + LOG_MAX_STRING_LEN];
  va_list args;
  va_start(args, msg);
  int check = vsnprintf(msgBuf, LOG_MAX_PREFIX_LEN + LOG_MAX_STRING_LEN, msg, args);
  if (check > 0 && static_cast<unsigned int>(check) <= sizeof(msgBuf))
    va_end(args);

  char logLineBuf[LOG_MAX_PREFIX_LEN + LOG_MAX_STRING_LEN];
  check = snprintf(logLineBuf, sizeof(logLineBuf), "%s", msgBuf);
  if (check > 0 && static_cast<unsigned int>(check) <= sizeof(logLineBuf))
    USART_DEBUG.println(logLineBuf);
}